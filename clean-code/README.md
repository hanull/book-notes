...
테스트가 가능한 시스템을 만드려고 노력하면 설계 품질이 높아진다. 이유는 아래와 같다.

1. SRP를 준수하는 클래스는 테스트가 쉽다.
2. 테스트 케이스가 많을수록 개발자가 쉽게 코드를 작성한다.
3. 결합도가 높으면 테스트 케이스를 작성하기 어렵다. 그러므로 DIP 원칙을 적용하고 DI, 인터페이스, 추상화 도구를 사용하여 결합도를 낮춘다.

따라서 테스트를 잘 작성하면 설계 품질이 높아진다.

### 리팩터링

테스트 케이스를 모두 작성했다면 코드와 클래스를 정리해도 괜찮다. 리팩토링 단계에서는 SW 설계 품질을 높여야한다.
...
- 관심사를 모듈로 나눈다.
- 클래스, 메소드 크기를 줄인다.
- 변수, 메소드 이름을 더 나은 것으로 수정한다.

### 중복을 없애라

중복은 추가 작업, 추가 위험, 불필요한 복잡도를 뜻하기 때문에 좋은 설계를 위해 중복을 없애야 한다.

### 표현하라

코드는 개발자의 의도를 분명히 표현해야 한다. 개발자가 코드를 명확하게 짤수록 다른 사람이 그 코드를 이해하기 쉬워지고, 그래야 결함이 줄고 유지보수 비용이 적게 든다.

1. 좋은 이름을 선택한다. 즉 기능을 이해하기 쉬운 이름으로 지어야 한다.
2. 클래스와 메서드 크기를 가능한 줄인다. 작을 수록 이름 짓기도 쉽고, 이해하기 쉽다.
3. 표준 명칭을 사용한다. 예를 들어, 클래스가 COMMAND, VISITOR과 같은 표준 패턴을 사용해 구현된다면 클래스 이름에 패턴 이름을 넣어준다.
4. 단위 테스트 케이스를 꼼꼼하게 작성한다. 테스트 케이스를 잘 만들면 클래스의 기능을 한눈에 파악할 수 있다.

무엇보다 가장 중요한 것은 `표현하고자 하는 노력` 이다. 그러므로 함수와 클래스에 조금 더 시간을 투자하자.

### 클래스와 메서드 수를 최소로 줄여라

위의 원칙을 잘 준수하면 좋지만, 극단으로 치달으면 오히려 관리가 어려워질 수 있고 득보다 실이 많아질 수 있다.

클래스와 메서드 수를 줄이는 궁극적인 목적은 효율적인 시스템을 개발하기 위함 임을 명심하자.
